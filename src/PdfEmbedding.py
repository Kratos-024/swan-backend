import io
from dotenv import load_dotenv
from langchain_huggingface import HuggingFaceEmbeddings
load_dotenv()
import numpy as np
import os
from langchain_community.document_loaders import PyPDFLoader
from langchain_text_splitters import RecursiveCharacterTextSplitter
from langchain_community.vectorstores import FAISS
from datetime import date

class PDFEmbed:
    def __init__(self,model_path,device,myDriveInst):
        self.embeddings = []
        self.model_path = model_path
        self.emebdding_model = HuggingFaceEmbeddings(
            model_name=self.model_path,
            model_kwargs={'device':device},
            encode_kwargs={'normalize_embeddings':True}
        )
        self.vector_store_file_id=None
        self.myDrive = myDriveInst
        self.pdf_path_name=''
    def createEmbedding(self, fileId):
        pdf_loader = PyPDFLoader(self.pdf_path_name)
        pdf_docs = pdf_loader.load()
        pdf_10_pages = pdf_docs[:10]
        
        splitter = RecursiveCharacterTextSplitter(chunk_size=100, chunk_overlap=20)
        chunks = splitter.split_documents(pdf_10_pages)
        new_vector_store = FAISS.from_documents(chunks, self.emebdding_model)
        
        metadata = {
            'fileId': fileId,
            'fileName': self.pdf_path_name,
            'date': date.today()
        }
        
        pdf_data = {
            "embedding": new_vector_store,
            "metadata": metadata,
        }
        np.save('pdf_vector.npy', pdf_data)
        self.myDrive.upload_file('pdf_vector.npy',)
        


    def create_pdf_from_buffer(self,pdf_buffer,pdf_file_name):
        pdf_bytes = bytes(pdf_buffer)
        buffer = io.BytesIO()
        buffer.write(pdf_bytes)
        buffer.seek(0)
       
        try:
            with open(pdf_file_name, 'wb') as f:
                f.write(buffer.getbuffer())
            self.pdf_path_name = pdf_file_name
        except IOError as e:
            print(f"error in upload_pdf: {e}")
        finally:
            buffer.close()
        return self.myDrive.upload_file(self.pdf_path_name)
   
    def search_query(self):
        file_present = self.myDrive.get_all_vectors()
        print(file_present)
                
                
        
        



        



 